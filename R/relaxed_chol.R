...
#  L_cscs = varband(S, lambda = lambda, init = L, lasso = TRUE, w = FALSE )
  for (i in 1 : p)
  {
    ...
    if (penalty == "MCP")
    {
      L[i , index] = RC_i(i, A = A, gamma = gamma, lambda = lambda, maxiter = maxiter, eps = eps,
                          initx = L[i, index], penalty = "MCP")
    }else{
      L[i , index] = RC_i(i, A = A, gamma = 0, lambda = lambda, maxiter = maxiter, eps = eps,
                          initx = L[i, index], penalty = "lasso")
    }
  }
  return(L)
}
...
##### Tuning using BIC
relaxchol_BIC <- function(X , gamma = 0.2, P , lamlist = NULL, nlam = 60, mu, flmin = 1e-2, maxiter = 100, penalty = c("lasso", "MCP"), eps = 1e-4, ebic_gam = 0.25)
{
  n <- nrow(X)
  p <- ncol(X)
  ...
  for (i in 1 : dim(theta)[1])
  {
    ...
    #   cat("gamma is", theta[i,2], "\n")
    #   cat("lambda is", theta[i,1], "\n")
    initL = storage
    bic.fit[i] = n/2 * sum(diag(P %*% S %*% t(P) %*% crossprod(initL))) -
      n * determinant(initL, logarithm = TRUE)$modulus[1] + sum(initL != 0) * log(max(n,p))
    ebic.fit[i] = n/2 * sum(diag(P %*% S %*% t(P) %*% crossprod(initL))) -
      n * determinant(initL, logarithm = TRUE)$modulus[1] + sum(initL != 0) * log(n) + 4 * sum(initL != 0) * log(p) * ebic_gam
  }
  ...
  return(list(gamma = gamma.bic, lambda = lambda.bic, bic.fit = bic.fit, lambda.ebic = lambda.ebic,
...
relaxchol_path <- function(X, P, gamma, maxiter = 100, eps = 1e-4, penalty = c("lasso", "MCP"),
  ...
  for (i in seq(ntheta)) {
   ...
   {
      result[, , i] <- relaxchol(X = X, P = P, gamma = theta[i, 2] , lambda = theta[i, 1], maxiter = maxiter,
                                 eps = eps, initL = result[, , i - 1], penalty = penalty)
...
relaxchol_CV <- function(X , gamma, P , lamlist = NULL, nlam = 60, flmin = 1e-2, folds = NULL, nfolds = 5,
                         maxiter = 100, penalty = c("lasso", "MCP"), eps = 1e-4)
{
  n <- nrow(X)
  p <- ncol(X)
  ...
  for (i in seq(nfolds)) {
    ...
    path_fit <- relaxchol_path(X = x_tr, P = P, gamma = gamma, lamlist = lamlist, maxiter = maxiter , eps = eps,
                               penalty = penalty)$path
  ...
  return(list(errs_fit = errs_fit, folds = folds, lamlist = lamlist, lambda = theta[ibest_fit, 1],
              gamma = theta[ibest_fit, 2],
